...
HULL_INPUT SHADOW_VERTEX_MAIN(VERTEX_INPUT input)
{
    HULL_INPUT output;
    output.position = float4(input.position, 1.0f);
    return output;
...
PatchTess SHADOW_CONSTANT_HULL(InputPatch<HULL_INPUT, 25> patch, uint patchID : SV_PrimitiveID)
{
    PatchTess output;
    output.EdgeTess[0] = output.EdgeTess[1] = 
    output.EdgeTess[2] = output.EdgeTess[3] = 
    output.InsideTess[0] = output.InsideTess[1] = 1.f;
    return output;
}
...
[outputtopology("triangle_cw")]
[outputcontrolpoints(25)]
[patchconstantfunc("SHADOW_CONSTANT_HULL")]
[maxtessfactor(1.f)]
DOMAIN_INPUT SHADOW_HULL_MAIN(InputPatch<HULL_INPUT, 25> p,
    uint i : SV_OutputControlPointID, uint patchId : SV_PrimitiveID)
{
    DOMAIN_INPUT output;
    output.position = p[i].position;
...
[domain("quad")]
PIXEL_INPUT SHADOW_DOMAIN_MAIN(PatchTess patchTess,
    float2 domainLocation : SV_DomainLocation,
    const OutputPatch<DOMAIN_INPUT, 25> patch)
{
    PIXEL_INPUT output;
